 ...
 *
 */
class IndexResult extends \TYPO3\CMS\Extbase\Persistence\Generic\QueryResult {
	...
	protected function initializeIndex() {
		if (!is_array($this->indexResult)) {
			...
			$this->indexResult = $database->exec_SELECTgetRows('*', IndexerService::TABLE_NAME, 'foreign_table = "tx_news_domain_model_news" AND foreign_uid IN (' . implode(',', $newsIds) . ') AND start_date > ' . time(), '', 'start_date ASC');
		}
	}
